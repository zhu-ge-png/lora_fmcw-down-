...
namespace gr
{
    namespace lora_sdr_fmcw_0
    {
                        ...
                        gr::io_signature::make2(1, 2, sizeof(gr_complex), sizeof(float)))
        {
            ...
            m_n_up_req = static_cast<uint8_t>(std::max<int>(1, static_cast<int>(preamble_len) - 3));
            ...
            symbol_cnt = 0;
            k_hat = 0;
            preamb_up_vals.resize(m_preamb_len, 0);
                                          ...
                                          gr_vector_void_star &output_items)
        {
            ...
            case DETECT:
            {
                ...
                if (symbol_cnt == (int)(m_n_up_req))
                {
                    ...
                    std::cout << "[frame_sync0 detect_lock] in=" << nitems_read(0)
                              << " k_hat=" << k_hat
                              << std::endl;
                    const int initial_handoff_stride_sym =
                        preamble_up_spacing_after(static_cast<int>(m_n_up_req) - 1);
                    memcpy(&net_id_samp[0],
                           &in[int((initial_handoff_stride_sym - 0.25f) * m_samples_per_symbol - k_hat * m_os_factor)],
                           sizeof(gr_complex) * 0.25 * m_samples_per_symbol);

                    items_to_consume = m_os_factor * ((int)(initial_handoff_stride_sym * m_number_of_bins - k_hat));
                }
                else
                    items_to_consume = m_samples_per_symbol;
                items_to_output = 0;
            ...
            case SYNC:
            {
                ...
                case NET_ID1:
                {
                    const bool current_symbol_is_preamble_up =
                        (bin_idx == 0 || bin_idx == 1 || (uint32_t)bin_idx == m_number_of_bins - 1);
                    const int absorbed_up_idx = static_cast<int>(m_n_up_req) + additional_upchirps;
                    if (current_symbol_is_preamble_up && absorbed_up_idx < static_cast<int>(m_preamb_len))
                    {
                        preamb_up_vals[absorbed_up_idx] = bin_idx;
                        memcpy(&preamble_raw[m_number_of_bins * absorbed_up_idx],
                               &in_down[0],
                               m_number_of_bins * sizeof(gr_complex));
                        memcpy(&preamble_raw_up[m_samples_per_symbol * absorbed_up_idx],
                               &in[(int)(m_os_factor / 2) + k_hat * m_os_factor],
                               m_samples_per_symbol * sizeof(gr_complex));

                        additional_upchirps++;
                        const int next_handoff_stride_sym = preamble_up_spacing_after(absorbed_up_idx);
                        memcpy(&net_id_samp[0],
                               &in[int((next_handoff_stride_sym - 0.25f) * m_samples_per_symbol)],
                               sizeof(gr_complex) * 0.25 * m_samples_per_symbol);
                        items_to_consume = next_handoff_stride_sym * m_samples_per_symbol;
                        break;
                    }
                ...
                case QUARTER_DOWN:
                {
                    ...
                    const uint32_t active_up_count =
                        static_cast<uint32_t>(std::min<int>(m_preamb_len, static_cast<int>(m_n_up_req) + additional_upchirps));
                    std::cout << "[frame_sync0 sync_down] in=" << nitems_read(0)
                              << " down_bin=" << down_val
                              << " sync_down_cfo_int=" << sync_down_cfo_int
                              << " absorbed_upchirps=" << (int)additional_upchirps
                    ...
                    CFO_int_correc.resize(active_up_count * m_number_of_bins);
                    for (uint32_t n = 0; n < active_up_count * m_number_of_bins; n++)
                    {
                    ...
                    sfo_corr_vect.resize(active_up_count * m_number_of_bins, 0);
                    for (uint32_t n = 0; n < active_up_count * m_number_of_bins; n++)
                    {
                    ...
                    corr_preamb.resize(active_up_count * m_number_of_bins, 0);
                    // apply sto correction
                    for (uint32_t i = 0; i < active_up_count * m_number_of_bins; i++)
                    {
                    ...
                    // apply cfo correction
                    volk_32fc_x2_multiply_32fc(&corr_preamb[0], &corr_preamb[0], &CFO_int_correc[0], active_up_count * m_number_of_bins);
                    for (uint32_t i = 0; i < active_up_count; i++)
                    {
                    ...
                    volk_32fc_x2_multiply_32fc(&corr_preamb[0], &corr_preamb[0], &sfo_corr_vect[0], active_up_count * m_number_of_bins);
                    ...
                    else if (abs(netid1 - (int32_t)m_sync_words[0]) > 2) // wrong id 1, (we allow an offset of 2)
                    {
                        ...
                        if (abs(netid1 - (int32_t)m_sync_words[1]) <= 2)
                        {
                            ...
                            for (int i = static_cast<int>(m_preamb_len) - 2; i < static_cast<int>(active_up_count); i++)
                            {
